...
//   c. List of doctors with their name and the number of times they are prescribed. Show list higher
//   order first.
    ...
    patients.forEach(patient => {
        patient.appointments.forEach((appointment) => {
            ...
            if (!doctors.has(doctor.name)) {
                doctors.set(doctor.name, 0);
            }
            let previousValue = doctors.get(doctor.name);
            doctors.set(doctor.name, previousValue+1);
        });
    });
    ...
    // descending order
    const higherOrderFirst = [...doctors].sort((a,b)=> b[1]- a[1]);

    console.log(higherOrderFirst);
