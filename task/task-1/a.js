...
// -------------------- a -------------------------------
// a. Retrieve a list of all medications prescribed for patients with asthma and their refill history.
const patients = task.patients;
patients.forEach((patient) => {
    const condition = patient.medical_history.conditions;
    condition.forEach((diseaseDetails) => {
        if(diseaseDetails.name === 'Asthma')
        {
            const medications = diseaseDetails.medications;
            medications.forEach((medics) => {
                console.log(`${patient.name} has been prescribed ${medics.name} for Asthma`);
