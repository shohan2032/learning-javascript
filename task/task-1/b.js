...
//   b. List all medications that have been refilled more than once within the last three months, along
//   with the associated patient and their doctor's details.
    const patients = task.patients;
    const currentDate = new Date('2025-01-08'); 
    const sixMonthsAgo = new Date();
    sixMonthsAgo.setMonth(currentDate.getMonth() - 6);
    // console.log(currentDate.toLocaleDateString());
    // console.log(sixMonthsAgo.toLocaleDateString());
    const refillsMoreThanSixMonths = new Map();

    patients.forEach(patient => {
        patient.medical_history.conditions.forEach(disease => {
            const medications = disease.medications;
            medications.forEach((medics) => {
                if (!refillsMoreThanSixMonths.has(medics.name)) {
                    refillsMoreThanSixMonths.set(medics.name, 0);
                }
            });
        });
    });

    // console.log(refillsMoreThanSixMonths);
    patients.forEach((patient) => {
        console.log();
        ...
        const condition = patient.medical_history.conditions;
        condition.forEach((diseaseDetails) => {
            const medications = diseaseDetails.medications;
            medications.forEach((medics) => {
                const refills = medics.refills;
                refills.forEach((data) => {
                    const date = new Date(data.date);
                    if(date >= sixMonthsAgo){
                        // console.log(`Dhukse -> ${data.date}`);
                        let previousValue = refillsMoreThanSixMonths.get(medics.name);
                        refillsMoreThanSixMonths.set(medics.name,previousValue+1);
                        // console.log(refillsMoreThanSixMonths);
                    } else {
                        // console.log(`Dhuke Nai -> ${data.date}`)
                    }
                })
            })
        });
        ...
        appointments.forEach((data)=>{
            ...
            console.log(data.doctor.specialty)
            console.log(data.doctor.location)
        })
    })

    console.log(refillsMoreThanSixMonths);

    refillsMoreThanSixMonths.forEach((value,key) => {
        if(value >= 2){
            console.log(key);
        }
    })
